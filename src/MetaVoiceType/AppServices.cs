...
public static class AppServices
{
    ...
    public static void Initialize(string[] args, StartupOptions options)
    {
        ...
        Host = Microsoft.Extensions.Hosting.Host.CreateDefaultBuilder(args).UseSerilog().ConfigureServices(services =>
        {
            ...
            services.AddSingleton<ITextInsertionService>(provider => provider.GetRequiredService<WindowsTextInsertionService>());
            services.AddSingleton<IKeyboardInputSimulator>(provider => provider.GetRequiredService<WindowsTextInsertionService>());
